...
export function insertDownloadButton(button: HTMLButtonElement): Result<boolean, PageError> {
    ...
    if (!fullscreenButton) {
        return err(domElementNotFoundError('Fullscreen button not found', 'button[aria-label="全画面表示"]'));
    }

    const controlBarButtonGroup = fullscreenButton.parentElement;
    if (!controlBarButtonGroup) {
        return err(domElementNotFoundError('Control bar button group not found', 'fullscreenButton.parentElement'));
    }
