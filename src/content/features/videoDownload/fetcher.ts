 ...
 */
function replaceUrlsInPlaylist(
  playlistText: string,
  baseUrl: string,
): { modifiedPlaylist: string; segmentUrls: { url: string; filename: string }[] } {
...
export async function downloadSegmentsForMux(
  ...
  try {
    ...
    if (segmentUrls.length === 0) {
      return err({
        type: 'UNKNOWN_ERROR',
        message: 'No segments found in media playlist',
        cause: null,
      });
    }
    ...
    if (hasError) {
      return err({
        type: 'FETCH_ERROR',
        message: 'Failed to download segments',
        cause: errorDetails,
      });
    }
