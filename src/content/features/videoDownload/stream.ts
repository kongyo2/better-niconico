...
export function getMasterUrl(): DownloadResult<string> {
    ...
    const masterEntry = resources.find(r =>
        r.name.includes('.m3u8') &&
        r.name.includes('playlists/variants/')
    );
    ...
    const anyM3u8 = resources.find(r =>
        r.name.includes('.m3u8') &&
        !r.name.includes('playlists/media/') // Exclude media playlists
    );
        ...
        if (text && text.includes(targetText)) {
            ...
            if (match) {
                ...
                if (parts.length >= 2) {
                    let url = parts[1];
                    url = url.replace(/\)$/, '');
                    if (url.startsWith('https://') && (url.includes('.m3u8') || url.includes('delivery.domand.nicovideo.jp'))) {
                        return ok(url);
                    }
                }
            }
    ...
    if (!playerArea) {
        return err({ type: 'MASTER_URL_NOT_FOUND', message: 'Master URL not found in network logs or system messages' });
    }
 ...
 */
export async function getVariantStreams(masterUrl: string): Promise<DownloadResult<VideoAudioStreams>> {
    try {
        const response = await fetch(masterUrl);
        if (!response.ok) {
            return err({ type: 'FETCH_ERROR', message: `Failed to fetch master playlist: ${response.status} ${response.statusText}`, cause: response });
        }
        ...
        if (videos.length === 0) {
            return err({ type: 'UNKNOWN_ERROR', message: 'No video streams found in master playlist', cause: text });
        }
        ...
        if (audios.length > 0) {
            ...
            return err({ type: 'UNKNOWN_ERROR', message: 'No audio streams found in master playlist', cause: text });
        }
